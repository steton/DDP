...
public abstract class AbstractService {
	...
	public enum ServiceStatus {
		...
		CONNECTED_TO_CLUSTERMANAGER,
		DISCONNECTED_FROM_CLUSTERMANAGER,
		NOT_YET_CONFIGURED_IN_CLUSTERMANAGER,
		UNKNOWN;
	};
	...
	protected void startInternalServices() throws Exception {
		log.debug("Start service subscriber.");
	...
	public ServiceStatus getLocalServiceStatus() {
	...
	private void startServiceSubscriber() {
		...
		try {
			...
			SchedulerUtil sched = new SchedulerUtil(schedulerPolicy, () -> {
					...
					try {
						...
						if(rcr != null) {
							if(rcr instanceof RemoteCallResultInterface) {
								...
								localServiceStatus = ServiceStatus.CONNECTED_TO_CLUSTERMANAGER;
							}
							else {
								log.debug(String.format("Invalid result."));
								localServiceStatus = ServiceStatus.DISCONNECTED_FROM_CLUSTERMANAGER;
							}
						}
						else {
	...
	private ServiceStatus localServiceStatus = ServiceStatus.UNKNOWN;
